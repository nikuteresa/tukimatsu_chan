  ...
  describe '#last_business_day_of_month' do
    ...
    context '月末が営業日でない場合' do
      before do
        ...
        allow(calculator).to receive(:business_day?).with(any_args).and_return(false)
      end
  ...
  describe '#last_business_day_of_current_month?' do
    context '当日が最終営業日の場合' do
      before do
        ...
        # 4月29日は営業日
        allow(calculator).to receive(:business_day?).with(date).and_return(true)
        # 4月30日は非営業日
        allow(calculator).to receive(:business_day?).with(Date.new(2025, 4, 30)).and_return(false)
      end
    ...
    context '当日が営業日だが最終営業日ではない場合' do
      before do
    ...
    context '当日が営業日でない場合' do
      before do
        date = Date.new(2025, 4, 26) # 土曜日
        allow(calculator).to receive(:business_day?).with(date).and_return(false)
      end
      
      it 'falseを返す' do
        expect(calculator.last_business_day_of_current_month?(Date.new(2025, 4, 26))).to be false
      end
    end
  end
end